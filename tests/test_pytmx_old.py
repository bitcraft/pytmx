...
class TiledMapTest(unittest.TestCase):
    ...
    def test_build_rects(self):
        from contrib import pygame_adapter

        rects = pygame_adapter.build_rects(self.m, "Grass and Water", "tileset", None)
        self.assertEqual(rects[0], [0, 0, 240, 240])
        rects = pygame_adapter.build_rects(self.m, "Grass and Water", "tileset", 18)
